...
### カウントする条件（すべて満たしたときだけ数える）

| 条件 | 理由 |
|---|---|
| **会話ページ**（URLに `/status/<数字>` を含む） | TLで人気記事が何度も流れるのは正常 |
| **行き先が確定している** | `t.co` のままのリンクを数えると、全リンクが同一ホスト扱いになって全部が連投になる |
| **`SAFE_HOSTS` 以外** | 大手/正規ドメインは何度貼られても異常ではない |
| **引用元の中のリンクではない** | 後述。これを入れないと引用タブで必ず誤爆する |
| **同じポストからは1回だけ** | 1つのポストに同じリンクが2箇所あっても1件 |
| **登録可能ドメインが3件以上のポストに出現**（`SPAM_REPEAT_MIN`） | `a1.spam.top` と `a2.spam.top` は同一視。ホスト完全一致だとサブドメインを回すだけで回避される |

閾値に達すると、**既に描画済みの過去のポストにも遡って**バッジを付け直します。

### 引用元を数えてはいけない理由

引用タブ (`/status/<id>/quotes`) を開くと、並んでいる全ポストが**同じ引用元**を抱えています。
引用元の中のリンクをそのまま数えると、1つの投稿がポスト数ぶん重複カウントされて
**必ず閾値を超え**、無関係な引用元に🚨が付きます。会話ページで同じポストが何度も
引用されている場合も同じです。

検出したいのは「**別々の人が**同じリンクを貼っている」ことなので、同一投稿の複製である
引用元はそもそも証拠になりません。判定は `quoteRootOf()` で行います。

> ただし、同じリンクが**本体と引用元の両方**にある場合は「引用元由来ではない」として数えます。
> 本体に貼っているのはその人自身の行為だからです。

> ⚠️ この誤爆は「同じURLを複数のポストに描画する」修正（`pending` を配列化）を入れたことで
> 顕在化しました。それ以前は同じURLにつき最後の1件しか記録されず、カウントが伸びなかったためです。
> 「描画の修正」と「カウントの正しさ」が同じ `pending` を共有している点に注意。
...
TikTok Lite のようなポイント系アプリの招待リンク。**アプリ名の列挙では追いつかない**
（キャンペーンごとに新しいアプリが出る）ので、招待キャンペーンが必ず持つ**構造**で判定します。

| 方法 | 定義 | 例 |
|---|---|---|
| クエリパラメータ | `INVITE_PARAMS`（24種、小文字化して完全一致） | `invite_code`, `invitation_code`, `referral_code`, `referralCode`, `inviter_id`, `invited_by`, `share_code` … |
| パスセグメント | `INVITE_PATH_RE` | `/invite/AB12`, `/referral`, `/invite-friend/…` |
| アプリ固有 | `INVITE_RULES` | `lite.tiktok.com`（招待コードがパスに埋まるためホストで判定） |

### 採用しない条件（誤検出になるもの）

| 却下したもの | 理由 |
|---|---|
| `?ref=` 単体 | 普通のサイトが「参照元」の意味で使う（`ref=nav` 等） |
| `?promo_code=` | クーポンであって招待とは限らない |
| `tiktok.com/t/…` | Lite の招待だけでなく通常の動画共有リンクでもある |
| パスの部分一致 | `/reference/api` や `/2026/invitation-to-summit` を巻き込む。**1セグメント完全一致**のみ |

**SAFE_HOSTS でも判定します。** アフィリエイトを `amazon.co.jp` でも判定するのと同じで、
パラメータが付いているのは事実だからです（ヒューリスティックではない）。

ラベルは中立に「招待/紹介」。危険ではないので**画像は覆いません**（バッジのみ）。

---

## 🔗 短縮URL

**既知ホストリストのみ** — `SHORTENER_HOSTS`（約43ドメイン）。
bit.ly / tinyurl / cutt.ly / lnkd.in / linktr.ee / lit.link / x.gd など。

AppsFlyer / Branch / Adjust 等の**アプリ計測リンク**（`*.onelink.me`, `*.app.link`,
`*.bnc.lt`, `*.adj.st`, `*.page.link`, `*.sng.link` …）もここに含めます。
「行き先（どのアプリ／どのストア）が隠れている」点が短縮URLと同じで、
招待キャンペーンのリンクはほぼこれ経由だからです。招待コード付きなら 🎁 と 🔗 が両方出ます。
...
**唯一、同梱データではなく利用者の設定で決まるカテゴリ**です。`chrome.storage.sync` に保存し、
`setUserRules()` で判定側へ差し込みます。

| リスト | 効果 |
|---|---|
| `userCaution` | ⚠️ を出して画像を覆う |
| `userExclude` | そのドメインを**一切判定しない**（ページ取得もしない） |

`userExclude` は `SAFE_HOSTS` とは別物です。`SAFE_HOSTS` が「ヒューリスティックだけ止める」
のに対し、`userExclude` は全カテゴリを止めます。誤検出されたときに利用者が打てる唯一の手なので、
中途半端に効かせません。

入力は `normalizeDomain()` で正規化します（URLを丸ごと貼られる前提。スキーム・パス・ポート・
`www.` を落として小文字化）。`hostInList` がサブドメインを含めて一致させるので、
登録は登録可能ドメインの粒度で足ります。上限は各200件（`storage.sync` は1項目8KBまで）。

### 公開ブロックリストからの自動充填を採用しなかった理由

当初はここに URLhaus / PhishTank 等のデータを流し込む想定でしたが、やめました。

- **鮮度**: フィッシングドメインの寿命は数日。同梱スナップショットはストア審査を待つ間に古くなる。
  しかも死んだドメインは転売されて無関係のサイトになるため、古いリストは「効かない」だけでなく
  **無関係のサイトに警告を貼る**方向に劣化する。
- **性質**: 他の全カテゴリは「有料である」「アフィパラメータが付いている」「広告枠が12個ある」
  という、リンク自体を見れば確かめられる事実の記述。これだけは他人のドメインに対する評価になり、
  外したときの実害の種類が違う。
- **範囲**: 既知の悪性URLは t.co 側が先に弾いているため、X上での取り分は小さい。

第三者由来のものを将来戻すなら、**出典を添えた形**（"URLhaus に登録あり"）にすること。
それなら評価ではなく引用元のある事実になり、他のカテゴリと同じ土俵に乗ります。
配信はリモート取得（毎日更新）で行い、拡張機能に同梱しないこと。

> さらに以前は「不審URL(🧪)」というカテゴリで、IP直リンク・怪しいTLD・ブランドなりすまし等の
